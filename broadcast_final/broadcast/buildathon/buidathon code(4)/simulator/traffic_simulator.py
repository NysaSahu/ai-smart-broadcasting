...
def simulate(time_steps=20):
    """
    Main simulation loop using physics-based SINR (Cost-231 Hata model).
    SINR is degraded by both user interference and distance-based path loss.
    """
    results = []
    state = NetworkState()

    for t in range(1, time_steps + 1):
        users, distance_km = generate_users(t)
        content, redundancy = generate_content_request()

        sinr_linear = compute_sinr(users, distance_km)
        popularity, _, _, _ = state.get_state()
        ...
        state.update_state(popularity, sinr_linear, congestion, distance_km)
